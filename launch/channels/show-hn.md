...
What it's honestly not: not a no-code tool (you write Go), not a Rails/Django replacement (no ORM, no router opinion, no generators-for-everything), and not production-hardened at scale on the default store — SQLite is the zero-setup local default, and for production you swap your own store in behind the relevant module store interfaces (auth uses `WithSessionStore`). One thing to flag up front: `/admin` is an open dashboard today, not a gated login — the seeded `admin@local.test` / `changeme` credentials authenticate against the auth API (`POST /api/v1/auth/sessions`, with `tenant_id` in the body), not an admin login screen; put `/admin` behind your own auth before exposing the port. It's also early: v0.1.0 — our first public release; expect APIs to move — verified on Linux/x86_64, Go 1.26. Things will move; pin a commit if you need stability today.
