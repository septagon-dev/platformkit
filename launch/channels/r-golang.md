...
Honest scope: it's early (v0.1.0 — our first public release; expect APIs to move), verified on Linux/x86_64, Go 1.26, `modernc.org/sqlite v1.50.1`, fresh DB. `/admin` is an open dashboard right now, not a gated login. SQLite is the zero-setup default — for production at scale you put your own store behind the relevant module store interfaces (auth uses `WithSessionStore`). It's not a Rails/Django-style web framework: no ORM, no router opinion, no generators-for-everything. fx-based DI is a real tradeoff if you dislike that style; it's what makes the compose-and-swap work rather than being a slogan.
