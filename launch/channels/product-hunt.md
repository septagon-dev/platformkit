...
The honest version: it's early (v0.1.0 — our first public release; expect APIs to move) and we're not dressing that up. The default store is SQLite for a zero-setup first run — for production at scale you swap your own store in behind the relevant module store interfaces (auth uses `WithSessionStore`). `/admin` is an open dashboard today, not a gated login — the seeded `admin@local.test` / `changeme` credentials authenticate against the auth API (`POST /api/v1/auth/sessions`, with `tenant_id` in the body), not an admin login screen; put it behind your own auth before exposing the port. It's a Go codebase, not a no-code tool, and not a Rails/Django replacement.
