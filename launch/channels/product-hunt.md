...
The honest version: it's early (v0.0.0) and we're not dressing that up. The default store is SQLite for a zero-setup first run — for production at scale you swap your own store in behind the store port. `/admin` is an open dashboard today, not a gated login. It's a Go codebase, not a no-code tool, and not a Rails/Django replacement.
