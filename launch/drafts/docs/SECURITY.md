...
> `security@septagon.dev` is the intended private reporting address. Confirm the
> live contact in the published repository before relying on it.
...
See also: [open-core.md](open-core.md) for what the OSS security baseline
covers, [quickstart.md](quickstart.md) for the honest caveat about the open
`/admin` dashboard.
