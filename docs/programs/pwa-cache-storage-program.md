...
This program applies only to `septagon-dev/platformkit-*` repos and
`septagon-clients/*` repos.

Reference repositories under `ComumCowork/*` are not delivery targets.
They are used only to inspect prior patterns and recover historical
implementations.

## Goal

Make platformkit's PWA, offline, cache, and client-storage behavior
contract-driven across backend, frontend, business modules, apps, and
client repos.
...
- Business modules declare policy instead of hand-rolling cache logic.
- Client repos override policy through approved configuration only.
- Sensitive and tenant-scoped data must never rely on implicit
...
- Mutations default to `network-only` and are not queueable unless
  explicitly declared.
- Auth and other sensitive flows must not use durable persistence by
  default.
- Public pages are cacheable and offline-readable.
- Device control and realtime operations must not present fake stale
  control state.
...
#### Business Module Adoption Work

- Adopt shared PWA policy contracts in the flagship modules.
- Declare invalidation tags for flagship read models and mutations.
...
#### Business Module Event Mapping Work

- Map flagship module events to invalidation tags.
- Add examples for public page updates, profile updates, admin config
  changes, and device state changes.

### M5 Client Override Model

Goal: allow safe client-specific policy changes without framework forks.

#### Client Repository Work

- Define override schema for TTLs, offline enablement, install metadata,
  and notification policy.
- Provide reference override configs for `septagon`, `platformkit`, and
  `comumcowork`.
- Add validation for unsafe or contradictory overrides.

#### Runtime Override Enforcement
...
- Add backend policy and service worker regression tests.
- Add frontend storage purge, scoped key, and sensitive-persistence tests.
- Add business-module conformance checks for all PWA-capable modules.
- Add app-level installability, offline, and purge lifecycle checks.
- Add offline smoke, tenant-switch purge, and persistence safety checks.
...
4. Integrate offline and cache lifecycle coordination into the frontend
   runtime.
